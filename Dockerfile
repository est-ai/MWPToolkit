...
### 사용자 공간
RUN pyenv uninstall -f ${PYTHON_VER} && rm -rf $PYENV_ROOT ${HOME}/.pip && apt install liblzma-dev && mkdir -p ${HOME}/.local/bin && mkdir -p ${HOME}/.local/lib && mkdir -p ~/.pip && git clone https://github.com/pyenv/pyenv.git ${HOME}/.pyenv && pyenv install ${PYTHON_VER} && pyenv global ${PYTHON_VER} &&  echo "export PYENV_ROOT='$HOME/.pyenv'" >> ~/.bashrc && echo "export PATH='$HOME/.local/bin:$PATH'" >> ~/.bashrc && echo "export PATH='$PYENV_ROOT/bin:$PATH'" >> ~/.bashrc && echo "eval '$(pyenv init - --no-rehash)'" >> ~/.bashrc && echo '[global]\nindex-url=http://ftp.daumkakao.com/pypi/simple\ntrusted-host=ftp.daumkakao.com' >> ~/.pip/pip.conf && apt-get update && apt-get install -y apt-utils wget git gcc build-essential cmake libpq-dev libsndfile-dev libboost-system-dev libboost-thread-dev libboost-program-options-dev libboost-test-dev libeigen3-dev zlib1g-dev libbz2-dev liblzma-dev libsndfile1-dev libopenblas-dev libfftw3-dev libgflags-dev libgoogle-glog-dev libgl1-mesa-glx libomp-dev && pip install --upgrade pip && pip install python-mecab-ko==1.0.9 && pip install git+https://github.com/caffeinism/pororo.git && python -c 'from pororo import Pororo; Pororo(task="ner", lang="ko"); Pororo(task="dep_parse", lang="ko"); Pororo(task="pos", lang="ko");' && python -c "from transformers import ElectraTokenizer; tokenizer = ElectraTokenizer.from_pretrained('monologg/koelectra-base-v3-discriminator'); from transformers import BertTokenizer; BertTokenizer.from_pretrained('monologg/koelectra-base-v3-discriminator')"
...
COPY --chown=${USER}:${USER} ./kesarr/ ${HOME}/
USER ${USER}
...
RUN python3 -m pip install kubernetes==17.17.0 grpcio==1.37.0 grpcio-tools==1.37.0 pika==1.2.0 chardet==4.0.0  --user
